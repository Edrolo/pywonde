...
This client is generated against pydantic v2 and requires `pydantic >= 2`.

**Regenerate only with openapi-generator `v7.1.0` or later.** That is where the
`python` generator moved to pydantic v2; earlier versions emit
`validate_arguments` and `Field(const=True)`, which pydantic v2 rejects. This was
previously worked around by hand-patching the generated output (commit
`3ce2d7c`), which any regeneration silently reverted. The generator version is
pinned in `meta/generate-python.sh`.

Wonde publishes no OpenAPI document, so `meta/wonde.openapi.yaml` is
hand-authored against <https://docs.wonde.com/docs/api/sync/> and drifts as Wonde
edits the docs. `meta/DRIFT.md` records the last reconciliation.

### Reading the raw response body

`ApiClient.last_response` was removed in generator v7.x. Use the
`*_with_http_info` variants instead — they return an `ApiResponse` carrying both
`.data` (deserialized) and `.raw_data` (the raw HTTP body).
...
*GroupsApi* | [**list_school_groups**](docs/GroupsApi.md#list_school_groups) | **GET** /schools/{school_id}/groups | Get all groups for a school
...
## Documentation For Models
 ...
 - [ListSchoolGroups200Response](docs/ListSchoolGroups200Response.md)
